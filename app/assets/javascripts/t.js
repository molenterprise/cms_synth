...
( function() {
		...
		t.getParent = function(childNode) {
			parentNode = {};
			t.find(this, function(node, par) {
		...
		t.getPreviousNode = function(node) {
			if (node.id == 0)
				return null;
			parent = t.getParent(node);
			for (var i = 0; i < parent.children.length; i++) {
				if (parent.children[i].id == node.id)
					return i > 0 ? parent.children[i - 1] : t.getParent(parent);
			}
		};

		t.deleteNode = function(node) {
			if (node.id == 0)
				return;
			parent = t.getParent(node);
			for (var i = 0; i < parent.children.length; i++) {
		...
		t.bfs = function(node) {
			...
			var cur, callback, i, length, par, children, isConfigSet = arguments.length === 3, config = isConfigSet ? arguments[1] : {}, queue = isArray(node) ? node.slice(0) : [node], parents = [undefined], childrenName = getChildrenName(config);
		...
		//
		t.dfs = function(node) {
			var cur, par, children, ctrl, i, ret, isConfigSet = arguments.length === 3, nodes = isArray(node) ? node.slice(0).reverse() : [node], config = isConfigSet ? arguments[1] : {}, callback = arguments[ isConfigSet ? 2 : 1], parents = [], childrenName = getChildrenName(config);
		...
		//
		t.map = function() {
			var node = arguments[0], isConfigSet = arguments.length === 3, config = isConfigSet ? arguments[1] : {}, filter = config.filter, nodeFactory = arguments[ isConfigSet ? 2 : 1], ret = isArray(node) ? [] : undefined, last = function(l) {
				return l[l.length - 1];
			}, parentStack = [], childrenName = getChildrenName(config);

			t.dfs(node, config, function(n, par, ctrl) {
				var curParent = last(parentStack), newNode = nodeFactory(n, curParent ? curParent.ret : undefined);
		...
		t.filter = function(node) {
			var isConfigSet = arguments.length === 3, nodeFactory = arguments[ isConfigSet ? 2 : 1], config = isConfigSet ? arguments[1] : {};
			return t.map(node, {
				filter : true,
		...
		t.stroll = function(tree1, tree2) {
			var i, node2, isConfigSet = arguments.length === 4, callback = arguments[ isConfigSet ? 3 : 2], config = isConfigSet ? arguments[2] : {}, childrenName = getChildrenName(config), nodes2 = isArray(tree2) ? tree2.slice(0).reverse() : [tree2], len = function(a) {
		...
		t.find = function(tree) {
			var found, isConfigSet = arguments.length === 3, callback = arguments[ isConfigSet ? 2 : 1], config = isConfigSet ? arguments[1] : {};
		...
		_dfsPostOrder = function(nodes, config, callback) {
			var cur, par, ctrl, node, last = function(l) {
				return l[l.length - 1];
			}, ret = [], stack = [{
				node : nodes.pop(),
				index : 0,
